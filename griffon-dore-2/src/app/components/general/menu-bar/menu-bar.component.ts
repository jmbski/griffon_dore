...
import { ANGULAR_COMMON, LayoutChangeObserver$, UseMobile } from '@app/common';
import { ElementSelector, WSMenuItem } from '@app/models'; 
...
export class MenuBarComponent {
    ...
    public calculateMaxPossibleHeightOfMenuItems() {
        const buttonHeight: number = 40; // 2 rem min height, + 0.5rem padding
        const pageWidth: number = window.innerWidth;
        
        const { bottom } = this.el.nativeElement.querySelector('.gdo-menubar')?.getBoundingClientRect() ?? { bottom: 0 };
        const svgHeight: number = (pageWidth - 32) / 4;
        const spacerHeight: number = 32; // 2rem
        ...
        const maxHeight = maxMenuItems * buttonHeight;
    ...
    public updateMenuScrolling() {
        ...
        const maxPossible = this.calculateMaxPossibleHeightOfMenuItems();
        
        if(this.containerElement && maxPossible > window.innerHeight) {
            ...
            if(containerElement && menubarItems.length > 0) {
                ...
                const maxHeight = pageHeight - top - height + svgHeight + spacerHeight;
